...
def normalize_data(indir, norm_outdir, filelist, mean, std, feature_dimension=10):
    with open(filelist,'r') as f:
        for filename in f:
            print(filename)
            filepath = os.path.join(indir, filename.strip())
            outfilepath = os.path.join(norm_outdir, filename.strip())
            data_mat= np.load(filepath)
...
def denormalize_data(indir, denorm_outdir, filelist, mean, std, feature_dimension=10):
    with open(filelist,'r') as f:
        for filename in f:
            filepath = os.path.join(indir, filename.strip())
            outfilepath = os.path.join(denorm_outdir, filename.strip())
            data_mat= np.load(filepath)
...
if __name__=="__main__":
    ...
    curr_dir = os.getcwd()
    indir = "/home/ubuntu/mnt1/tacotron-keitho/training"
    outdir= "/home/ubuntu/mnt1/data/norm_stats/"
    norm_outdir = "/home/ubuntu/mnt1/data/norm_f0_feats/"
    denorm_outdir = "/home/ubuntu/mnt1/data/denorm_f0_feats/"
